...
struct CalculatorView: View {
    ...
    private func negate() {
        ...
        display = value == 0 ? "0" : String(value * -1)
    }

    private func percent() {
        guard let value = Double(display) else { return }
        display = String(value / 100.0)
    }
...
enum KeyStyle {
    ...
    var background: Color {
        switch self {
        case .number: return Color.white
        case .utility: return Color(.systemGray4)
        case .operatorKey: return Color.orange
        }
    }

    var foreground: Color {
        switch self {
        case .number, .operatorKey: return .black
